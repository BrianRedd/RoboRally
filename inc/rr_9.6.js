...
$(document).ready(function () {
	...
	var bdorient = new Array(); //board orientation array
	var tile = new Array(); //tile array
	var Robot = new Array(); //'Bot's
	var RoboMod = new Array(); //'Bot models
	var Flag = new Array();//Flags object
	var Face = new Array();//Facing object
	...
	var activeOnReg = new Array(); //active on register - used for tiles that are register activated
	for (var i = 1; i < 6; i++) { //set five registers as individual arrays (so activated tiles can be assigned to it)
		activeOnReg[i] = new Array();
	};
	
	var hand = new Array(); //set hand array, used for each robot (up to nine source cards)
	var program = new Array(); //programs array, used for each robot (five register actions per 'bot)
	...
	function readyGo() { //Form selections done, time to start game
		...
		/*if (!pushermove) {
			$(".ph2").parent().css('display', 'none');
		};
		if (!gearmove) {
			$(".ph3").parent().css('display', 'none');
		};
		if (!crushermove) {
			$(".ph4").parent().css('display', 'none');
		};*/
	...
	RL$.find(".closer").click(function() {
		RL$.slideToggle();
	})
	Instructions$.find(".closer").click(function() {
		Instructions$.fadeToggle();
	})
	...
	function pitborder() {
		...
		//tile[gridy + 1] = new Array
		for (var i = -1; i <= gridy; i++) {
			//tile[i][-2] = new tileObj("0", 0);
			//tile[i][gridx + 1] = new tileObj("0", 0);
			/*if (i == -1 || i == (gridy + 1)) {
				tile[i][-1] = new tileObj("0", 0);
				tile[i][gridx] = new tileObj("0", 0);
			} else {*/
		...
		for (var i = 0; i < gridx; i++) {
			/*tile[-2][i] = new tileObj("0", 0);
			tile[gridy + 1][i] = new tileObj("0", 0);
			if (i == -1 || i == gridx + 1) {
				tile[-1][i] = new tileObj("0", 0);
				tile[gridy][i] = new tileObj("0", 0);
			} else {*/
		...
		for (var i = 0; i < num; i++) {
			...
			if (i == 0) {
				...
				RMV$.find("#stat0").click(function() {
					...
					function() {
						GS$.find("#robot0").addClass("rbt_lyr_hover")
					},
					function() {
						GS$.find("#robot0").removeClass("rbt_lyr_hover")
					}
				);
				PC$.find("h2").text(Robot[0].name + " CONSOLE");
			} else {
				RMV$.find("#regs" + (i - 1)).after('<div class="robostats" id="stat' + i + '"></div>');
				RMV$.find("#stat" + i).append('<img src="' + Robot[i].thumb + '"/><div class="stats"></div>');
				RMV$.find("#stat" + i + " .stats").append('<span class="rdesig"></span><br/>Health: <div class="dmg"> </div><br/>Lives: <div class="lvs"></div><br/>Flags: <div class="flg"></div>');
				RMV$.find("#stat" + i + " .rdesig").text(Robot[i].name);
				RMV$.find("#stat" + i).after('<div class="robomoves" id="regs' + i + '"></div>');
			}
		};
		$("#robot0").draggable({ //player 'bot draggable to see beneath and around it; reverts to starting position automatically
			revert: true
		});
...
--------------------------------------------------------*/ 	
	...
	PC$.find(".closer").click(function() { //"closer" closes console
		openConsole();
	});
...
/*				var robotlbm = parseInt(temp.rlbm);
	...
	function GameOver(r) {
		...
		var temp = "<h3><em>" + Robot[r].name + "</em> WINS by capturing all " + nuflags + " flags!</h3>";
